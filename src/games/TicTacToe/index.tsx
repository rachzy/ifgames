...
const TicTacToe = () => {
  ...
  const returnSymbolByValue = (field: IGameField) => {
    if (field.value === "X") {
      return (
        <FaTimes color={field.color === null ? "" : field.color} size={42} />
      );
    }
    return (
      <BiCircle color={field.color === null ? "" : field.color} size={42} />
    );
  };
  ...
  const returnGameFields = () => {
    return gameFields.map((field) => {
      return (
        ...
        >
          {field.value ? returnSymbolByValue(field) : ""}
        </GameField>
      );
    });
  };
  ...
  if (game.started) {
    return (
      <MainWrapper>
        <div className={classes.gameContainer}>{returnGameFields()}</div>
      </MainWrapper>
    );
  }
  ...
  if (game.winner) {
    return (
      <MainWrapper>
        <Title color={game.winner.color}>
          {game.winner.nick} é o vencedor!
        </Title>
        <Button color={game.winner.color} onClick={startGame}>
          Jogar Novamente
        </Button>
      </MainWrapper>
    );
  ...
  return (
    <MainWrapper>
      ...
      <Subtitle>Jogo feito para 2 jogadores</Subtitle>
